...
考虑到 compose 函数 经常与 createStore 连用的用法，可以看出其主要就是对 createStore 方法进行增强，新增一些原生 Redux 没有的功能。

#### applyMiddleware.js

``` javascript
export default function applyMiddleware(...middlewares) {
  /* return 一个函数，它可以接受 createStore 方法作为参数，给返回 store 的 dispatch 方法再进行一次包装 */
